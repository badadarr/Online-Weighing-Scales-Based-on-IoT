    ...
    fetch('/status')
        ...
        .then(data => {
            ...
            if (accessElement && data.access_status) {
                ...
                if (data.access_status.includes('granted') || data.access_status.includes('welcome')) {
                    accessElement.classList.add('granted');
                } else if (data.access_status.includes('denied') || data.access_status.includes('unauthorized')) {
...
function updateScaleConfigStatus(data) {
    ...
    if (baseWeightElement) {
        baseWeightElement.textContent = data.baseWeight ? data.baseWeight.toFixed(3) + ' kg' : '0.000 kg';
    }
...
function quickTare() {
    if (!confirm('This will set the current reading as zero. Make sure the scale is empty. Continue?')) {
        return;
    }
...
function addUser() {
    ...
    if (!/^[0-9A-Fa-f\s]+$/.test(uid)) {
        showResponse('Invalid UID format. Please use hexadecimal characters only.', 'error');
        return;
    }
    ...
    .then(data => {
        if (data.success) {
            ...
            showResponse('Error adding user: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
    ...
    fetch('/api/rfid/users')
        ...
        .then(data => {
            if (data.success && data.users) {
                ...
                userListDiv.innerHTML = '<div class="user-item">No users found or error loading users</div>';
            }
        })
        .catch(error => {
            console.error('Error loading users:', error);
            userListDiv.innerHTML = '<div class="user-item">Error loading users. Please check connection.</div>';
        });
}
...
function displayUsers(users) {
    ...
    if (users.length === 0) {
        userListDiv.innerHTML = '<div class="user-item">No users registered yet</div>';
        return;
    }
    ...
    fetch('/api/config')
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById('calibrationFactor').value = data.calibrationFactor || 1.0;
                document.getElementById('stabilizationTime').value = data.stabilizationTime || 3;
                document.getElementById('weightThreshold').value = data.weightThreshold || 1.0;
            }
        })
        .catch(error => {
...
function saveScaleConfig() {
    const config = {
        ...
        calibrationFactor: parseFloat(document.getElementById('calibrationFactor').value) || 1.0,
        stabilizationTime: parseInt(document.getElementById('stabilizationTime').value) || 3,
        weightThreshold: parseFloat(document.getElementById('weightThreshold').value) || 1.0
    };
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            showScaleResponse('Error saving configuration: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
...
function performTare() {
    ...
    .then(data => {
        if (data.status === 'success') {
            showScaleResponse('Scale tared successfully! Current reading set to zero.', 'success');
        } else {
            showScaleResponse('Error performing tare: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
        console.error('Error performing tare:', error);
...
function calibrateScale() {
    const weight = prompt('Place a known weight on the scale and enter its value in kg:');
    if (!weight || isNaN(weight) || parseFloat(weight) <= 0) {
        showScaleResponse('Invalid weight value entered.', 'error');
        return;
    }
    
    if (!confirm(`Calibrating with ${weight} kg. Make sure this weight is correctly placed on the scale. Continue?`)) {
        return;
    }
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            showScaleResponse('Error calibrating scale: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
    ...
    fetch('/api/system-config')
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById('sessionTimeout').value = data.sessionTimeout || 5;
            }
        })
        .catch(error => {
...
function saveSystemConfig() {
    const config = {
        ...
        sessionTimeout: parseInt(document.getElementById('sessionTimeout').value) || 5
    };
    ...
    .then(data => {
        if (data.status === 'success') {
            showSystemResponse('System configuration saved successfully!', 'success');
        } else {
            showSystemResponse('Error saving configuration: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
...
function resetToDefaults() {
    if (!confirm('This will reset all configurations to default values. This action cannot be undone. Continue?')) {
        return;
    }
    ...
    .then(data => {
        if (data.status === 'success') {
            showSystemResponse('All configurations reset to defaults successfully!', 'success');
            setTimeout(() => {
                loadScaleConfig();
                loadSystemConfig();
            }, 1000);
        } else {
            showSystemResponse('Error resetting to defaults: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
